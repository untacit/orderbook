...
public class LocationResource {
    ...
    public List<LocationDTO> getAllLocations(@RequestParam(required = false) String filter) {
        if ("buyer-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where buyer is null");
            return StreamSupport
                .stream(locationRepository.findAll().spliterator(), false)
                .filter(location -> location.getBuyer() == null)
                .map(locationMapper::toDto)
                .collect(Collectors.toCollection(LinkedList::new));
        }
        if ("store-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where store is null");
            return StreamSupport
                .stream(locationRepository.findAll().spliterator(), false)
                .filter(location -> location.getStore() == null)
                .map(locationMapper::toDto)
                .collect(Collectors.toCollection(LinkedList::new));
        }
